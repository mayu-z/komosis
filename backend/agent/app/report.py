"""
PDF Report Generator for Komosis Agent.
...
The report contains:
  1. Header with Komosis branding and run metadata
...
logger = logging.getLogger("komosis.report")
...
def generate_report_pdf(results: dict[str, Any], output_path: Path) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        leftMargin=18 * mm,
        rightMargin=18 * mm,
        title=f"Komosis Run Report — {results.get('run_id', 'unknown')}",
        author="Komosis Autonomous CI/CD Healing Agent",
    )
    ...
    # ── 1. Title ────────────────────────────────────────────
    elements.append(Paragraph("Komosis — Autonomous CI/CD Healing Agent", title_style))
    ...
    elements.append(Paragraph(
        f"Generated by Komosis Autonomous CI/CD Healing Agent • {now}",
        small_style,
    ))
